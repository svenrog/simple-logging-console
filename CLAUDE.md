...
- A test project exists at `tests/Simple.Logging.Console.Tests` (xUnit). Run `dotnet test SimpleLogFormatter.slnx`. Benchmarks live in `benchmarks/Simple.Logging.Console.Benchmarks` (BenchmarkDotNet); run with `dotnet run -c Release --project benchmarks/… -- --filter '*'`.
